...
ln -sv $WEST_SIM_ROOT/Namd_Config/chignolin-ref_autopsf.psf structure.psf
ln -sv $WEST_SIM_ROOT/Namd_Config/chignolin-ref_autopsf.pdb structure.pdb
ln -sv $WEST_SIM_ROOT/Namd_Config/plumed_final.dat plumed_final.dat
ln -sv $WEST_SIM_ROOT/Namd_Config/par_all36m_prot.prm par_all36m_prot.prm
...
/data/homezvol2/prprabha/NAMD_2.14_Linux-x86_64-multicore/namd2 +p 48 md.conf > seg.log 
...
plumed driver --plumed plumed_final.dat --mf_dcd seg.dcd 
python $WEST_SIM_ROOT/westpa_scripts/calculate.py > $WEST_PCOORD_RETURN
#cat q.dat > $WEST_PCOORD_RETURN
...
rm -f md.conf seg.pdb \
  ...
  seg.restart.xsc seg.restart.xsc.old structure.psf toppar_water_ions_namd.str
